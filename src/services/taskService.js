...
//     try {
//       // First, get the current subactivity - USE THE LIST ENDPOINT to fetch all and filter
//       // Since GET on detail endpoint is not available, we fetch all subactivities
...
//       // Check if user already picked this task
...
//       // Update the subactivity with new picked_at using PUT
//       const updateData = {
//         ...subActivity,
//         picked_at: updatedPicks
//       };
      
//       // Use PUT to update the subactivity
...
//   // Get user's picked tasks - Query subactivities where picked_at contains user
...
//   // Start working on a task - Update user's pick record with started_at
//   async startTask(subActivityId, empCode) {
//     try {
//       // Get current subactivity from list endpoint
//       const allResponse = await api.get('/subactivity/');
//       const subActivity = allResponse.data.find(s => s.id === subActivityId);
...
//       // Find and update user's pick record
...
//   // Complete a task - Update user's pick record with completed_at and time spent
...
//       } else {
//         newProgress = 100; // For status-based tasks
//       }
...
//       // Update subactivity's completed quantity and progress
//       const updateData = {
//         ...subActivity,
//         picked_at: updatedPicks,
//         completed_quantity: newCompletedQty,
...
//       let newProgress = 0;
//       if (totalQty > 0) {
//         newProgress = Math.round((completedQuantity / totalQty) * 100);
//       }
...
//   // Remove a picked task (unpick)
//   async unpickTask(subActivityId, empCode) {
//     try {
//       // Get current subactivity from list endpoint
//       const allResponse = await api.get('/subactivity/');
//       const subActivity = allResponse.data.find(s => s.id === subActivityId);
...
//       // Remove user's pick record
//       const currentPicks = subActivity.picked_at || [];
//       const updatedPicks = currentPicks.filter(pick => pick.emp_code !== empCode);
...
//       return response.data;
//     } catch (error) {
//       console.error('Error unpicking task:', error);
//       throw error;
//     }
//   }
// }
