...
const CreateProject = () => {
  ...
  const handleAddSubActivity = async (e) => {
    ...
    // 🔥 MULTIPLE MODE
      ...
      const count = Number(newSubActivity.noofchainage) || 0;

      if (!count || !covered) {
        dispatch(showSnackbar({
          message: "Enter valid chainage and covered area",
          type: "error"
        }));
        return;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const projectData = {
        ...
        director_proposal_date: form.director_proposal_date ? form.director_proposal_date : null,
        project_confirmation_date: form.project_confirmation_date ? form.project_confirmation_date : null,
        ...
        {showAddCompanyModal && (
          ...
          >
            <motion.div
              ...
              onClick={(e) => e.stopPropagation()}
            >
              <h3 className="text-lg md:text-xl font-bold mb-4">Add New Company</h3>
              <input
                type="text"
              ...
              <form onSubmit={handleAddSubActivity}>
                ...
                <div className="space-y-3 md:space-y-4">
                  ...
                  <div className="flex gap-4 mb-4">
                    <button
                      type="button"
                      onClick={(e) => setNewSubActivity({ ...newSubActivity, activityType: "single" })}
                      className={`px-4 py-2 rounded-lg ${newSubActivity.activityType === "single"
                    ...
                    <button
                      type="button"
                      onClick={(e) => setNewSubActivity({ ...newSubActivity, activityType: "multiple" })}

                      className={`px-4 py-2 rounded-lg ${newSubActivity.activityType === "multiple"
                        ? "bg-blue-500 text-white"
                        : "bg-gray-200 dark:bg-gray-700"
                        }`}
                    >
                  ...
                  {newSubActivity.activityType === "single" ? (<> <input
                    type="text"
                  ...
                  />
                    ...
                    </select></>) : <div className="space-y-4"><input
                      type="text"
                      placeholder="Sub Activity Name"
                      value={newSubActivity.name}
                      onChange={(e) =>
                    ...
                    <select
                      value={newSubActivity.unit}
                      onChange={(e) =>
                        setNewSubActivity({ ...newSubActivity, unit: e.target.value })
                      }
                      className="w-full p-3 border rounded-xl"
                    >
                    ...
                    <input
                      ...
                      value={newSubActivity.startchainage}
                      onChange={(e) =>
                        setNewSubActivity({ ...newSubActivity, startchainage: e.target.value })
                      }
                      className="w-full p-3 border rounded-xl"
                    />
                    ...
                    <input
                      ...
                      value={newSubActivity.coveredarea}
                      onChange={(e) =>
                        setNewSubActivity({ ...newSubActivity, coveredarea: e.target.value })
                      }
                      className="w-full p-3 border rounded-xl"
                    />
                    <input
                      type="number"
                      placeholder="No. of Chainage"
                      value={newSubActivity.noofchainage}
                      onChange={(e) => {
                        setNewSubActivity({ ...newSubActivity, noofchainage: e.target.value })
      ...
      <form onSubmit={handleSubmit} className="space-y-6 md:space-y-8">
              ...
              >
                ...
                {showTlDropdown && (
                  <div className="absolute z-[9999] mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                    {(tlSearch
                      ? tls.filter((c) =>
                        c.name?.toLowerCase().includes(tlSearch.toLowerCase()) ||
                        c.emp_code?.toLowerCase().includes(tlSearch.toLowerCase())
                      )
                      : tls
                    ).length > 0 ? (
                      (tlSearch
                        ...
                        <div
                          ...
                          onClick={(e) => {
                            ...
                            setForm({
                              ...form,
                              assigned_to: tl.emp_code
                            });
                            setTlSearch(`${tl.name} - ${tl.emp_code}`); // show both
                            setShowTlDropdown(false);
                          }}
                          className="px-3 py-2 hover:bg-blue-50 cursor-pointer text-sm"
                        >
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {/* Total Length */}
            <div className="flex flex-col gap-1">
              <label className="text-xs text-gray-500">Total {" "}
                {sectorsList.filter((data) => data?.name.toLowerCase() == form.sector.toLowerCase())[0]?.unit} *</label>
              <div className="relative">
            ...
            <div className="flex flex-col gap-1 relative">
              ...
              {parseFloat(form.workorder_Amount) > 0 && (
                <div className="col-span-1 sm:col-span-2 lg:col-span-3 mt-2 p-3 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl absolute top-full w-full">
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    <div>
                      <p className="text-[10px] text-gray-500">Workorder Amount</p>
                      <p className="text-sm font-semibold text-gray-800">
                        ₹ {parseFloat(form.workorder_Amount).toLocaleString('en-IN', { minimumFractionDigits: 2 })} Lakhs
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] text-gray-500">GST ({form.igst_percentage}%)</p>
                      <p className="text-sm font-semibold text-blue-600">
                        ₹ {calculatedGST.igst.toLocaleString('en-IN', { minimumFractionDigits: 2 })} Lakhs
                      </p>
                    </div>
                    {/* <div>
                    <p className="text-[10px] text-gray-500">CGST ({form.cgst_percentage}%)</p>
                    <p className="text-sm font-semibold text-blue-600">
                      ₹ {calculatedGST.cgst.toLocaleString('en-IN', { minimumFractionDigits: 2 })} Lakhs
                    </p>
                  </div> */}
                    <div>
                            ...
                            <div className="space-y-2 md:space-y-3">
                              ...
                              {(() => {
                                ...
                                activityObj?.subActivities.forEach((sub, idx) => {
                                  ...
                                  grouped[sub.name].push({ ...sub, originalIndex: idx });
                                });

                                // Ab group wise render karo
                                const allRendered = [];
