...
//       animate={{ opacity: 1 }}
//       exit={{ opacity: 0 }}
//       className="fixed inset-0 bg-black/30 dark:bg-black/50 backdrop-blur-md z-[100] flex items-center justify-center"
//     >
//       <motion.div
...
//             />
//             <motion.div
//               className="absolute inset-2 border-4 border-blue-400 dark:border-blue-600 rounded-full border-t-transparent"
//               animate={{ rotate: 360 }}
//               transition={{ duration: 1.5, repeat: Infinity, ease: "linear" }}
//             />
//             <motion.div
//               className="absolute inset-4 border-4 border-blue-600 dark:border-blue-400 rounded-full border-b-transparent"
//               animate={{ rotate: -360 }}
...
//           <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{submessage}</p>
...
// // Map backend status to frontend for display
...
// // Map frontend unit to backend enum values
...
// // Get deadline status - FIXED: Now returns null for completed items
// const getDeadlineStatus = (deadline, isCompleted = false) => {
...
// // Calculate activity progress correctly (average of all sub-activities)
...
// // Calculate project progress correctly (average of all activities with weightage)
// const calculateProjectProgress = (activities) => {
//   if (!activities || activities.length === 0) return 0;
  
//   // If activities have weightage, use weighted average
//   const hasWeightage = activities.some(a => a.weightage && a.weightage > 0);
  
//   if (hasWeightage) {
//     const totalWeightage = activities.reduce((sum, a) => sum + (a.weightage || 0), 0);
//     if (totalWeightage === 0) return 0;
    
//     const weightedProgress = activities.reduce((sum, a) => {
//       return sum + ((a.progress || 0) * (a.weightage || 0) / 100);
//     }, 0);
...
//   // Store full project and activities
...
//   // Fetch only essential data first, then lazy load the rest
//   useEffect(() => {
...
//         // Then fetch essential reference data
//         await Promise.all([
...
//   // Lazy load activities and sub-activities after initial load
//   useEffect(() => {
...
//       setLoadingSubMessage("Fetching activity details...");
//       try {
//         await Promise.all([
...
//   // Find and set project from API or local store
//   useEffect(() => {
...
//   // Process project activities with memoization
//   useEffect(() => {
//     if (!project) return;
...
//           activityIds = project.activities;
          
//           activitiesList = activityIds
...
//               }))
//             }));
//         } else if (typeof project.activities[0] === 'object' && project.activities[0] !== null) {
...
//   const getProjectShortName = useCallback(() => {
...
//     if (project?.company_detail?.name) {
//       return project.company_detail.name;
//     }
...
//     if (project?.sub_company_detail?.name) {
//       return project.sub_company_detail.name;
//     }
...
//     if (project?.sector_detail?.name) {
//       return project.sector_detail.name;
//     }
...
//     if (project?.client_detail?.name) {
//       return project.client_detail.name;
//     }
...
//     return mapStatusToFrontend(project?.status || "Pending");
//   }, [project]);

//   const getProjectExtensionRequested = useCallback(() => {
...
//   // Optimized progress update with correct calculation - FIXED to not affect other sub-activities
...
//       // Calculate progress for this sub-activity only
...
//       // Calculate new activity progress using ALL sub-activities (including unchanged ones)
...
//       const isActivityCompleted = updatedSubs.every(sub => sub.progress === 100);
      
//       // Update activity if progress changed
...
//       // Calculate new project progress using ALL activities
//       const updatedActivities = getProjectActivities().map(a => 
//         a.id === activity.id ? { ...a, progress: activityProgress } : a
//       );
      
//       const projectProgress = calculateProjectProgress(updatedActivities);
...
//       // Update local state for this sub-activity only
...
//       // Recalculate activity progress using ALL sub-activities
//       const updatedSubs = activity.subactivities.map(sub => 
...
//       // Recalculate project progress using ALL activities
//       const updatedActivities = getProjectActivities().map(a => 
//         a.id === activity.id ? { ...a, progress: activityProgress } : a
//       );
...
//       case "DELAYED":
//         return "bg-red-100 text-red-600 border-red-200 dark:bg-red-900/20 dark:text-red-400";
//       case "HOLD":
...
//   // FIXED: Don't show deadline badge for completed items
...
//         initial={{ scale: 0.9 }}
//         animate={{ scale: 1 }}
//         className="text-xs px-2 py-1 bg-red-100 text-red-600 dark:bg-red-900/20 dark:text-red-400 rounded-full"
//       >
//         Overdue
//       </motion.span>
//     );
...
//         initial={{ scale: 0.9 }}
//         animate={{ scale: 1 }}
//         className="text-xs px-2 py-1 bg-orange-100 text-orange-600 dark:bg-orange-900/20 dark:text-orange-400 rounded-full"
//       >
//         {days} days left
//       </motion.span>
//     );
...
//         initial={{ scale: 0.9 }}
//         animate={{ scale: 1 }}
//         className="text-xs px-2 py-1 bg-yellow-100 text-yellow-600 dark:bg-yellow-900/20 dark:text-yellow-400 rounded-full"
//       >
//         {days} days left
//       </motion.span>
//     );
...
//         initial={{ scale: 0.9 }}
//         animate={{ scale: 1 }}
//         className="text-xs px-2 py-1 bg-green-100 text-green-600 dark:bg-green-900/20 dark:text-green-400 rounded-full"
//       >
//         {days} days left
//       </motion.span>
//     );
...
//     return <LoadingOverlay message={loadingMessage} submessage={loadingSubMessage} />;
//   }
...
//     >
//       {/* Custom Loading Overlay for submissions */}
//       <AnimatePresence>
//         {isSubmitting && (
...
//                         className="w-full p-3 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 transition-all"
//                         required
//                       />
//                     </motion.div>
...
//                           <Info size={12} />
//                           Progress will be tracked as percentage complete.
//                         </p>
//                       </motion.div>
...
//                         animate={{ x: 0, opacity: 1 }}
//                         transition={{ delay: 0.3 }}
//                         className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl text-sm text-blue-600 dark:text-blue-400 flex items-center gap-2"
//                       >
//                         <Info size={16} />
...
//               <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 dark:text-white line-clamp-1">
//                 {getProjectName()}
//               </h1>
...
//               className="flex-1 sm:flex-none bg-gray-800 dark:bg-gray-700 text-white px-3 sm:px-4 py-2 rounded-xl hover:bg-gray-900 dark:hover:bg-gray-600 transition-all flex items-center justify-center gap-2 text-sm"
//             >
//               <FileText size={16} />
...
//                 className="flex-1 sm:flex-none bg-blue-600 text-white px-3 sm:px-4 py-2 rounded-xl hover:bg-blue-700 transition-all flex items-center justify-center gap-2 text-sm"
//               >
//                 <Calendar size={16} />
...
//           animate={{ y: 0, opacity: 1 }}
//           transition={{ delay: 0.2 }}
//           className={`rounded-xl sm:rounded-2xl p-3 sm:p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 ${
//             getProjectStatus() === "DELAYED" ? "bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800" :
//             getProjectStatus() === "COMPLETED" ? "bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800" :
...
//               >
//                 <AlertCircle className="text-red-600 dark:text-red-400" size={isMobile ? 20 : 24} />
//               </motion.div>
...
//                 transition={{ duration: 0.5, repeat: Infinity, repeatDelay: 2 }}
//               >
//                 <CheckCircle2 className="text-green-600 dark:text-green-400" size={isMobile ? 20 : 24} />
//               </motion.div>
//             ) : (
//               <motion.div
//                 animate={{ rotate: 360 }}
...
//               >
//                 <Clock className="text-blue-600 dark:text-blue-400" size={isMobile ? 20 : 24} />
//               </motion.div>
//             )}
//             <div>
//               <p className="font-semibold text-sm sm:text-base text-gray-900 dark:text-white">
//                 Project Status: <span className={
//                   getProjectStatus() === "DELAYED" ? "text-red-600 dark:text-red-400" :
//                   getProjectStatus() === "COMPLETED" ? "text-green-600 dark:text-green-400" :
//                   "text-blue-600 dark:text-blue-400"
//                 }>{getProjectStatus()}</span>
//               </p>
//               <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">
//                 {getDeadlineStatus(completionDate, getProjectStatus() === "COMPLETED") === "OVERDUE" ? "Project is overdue" :
//                  getDeadlineStatus(completionDate, getProjectStatus() === "COMPLETED") === "TODAY" ? "Project due today!" :
//                  getDeadlineStatus(completionDate, getProjectStatus() === "COMPLETED") === "CRITICAL" ? "Project deadline critical" :
...
//             <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Completion Date</p>
//             <p className="font-semibold text-sm sm:text-base text-gray-900 dark:text-white">{formatDate(completionDate)}</p>
//             <div className="mt-1">{getDeadlineBadge(completionDate, getProjectStatus() === "COMPLETED")}</div>
//           </div>
//         </motion.div>
...
//           className="bg-white dark:bg-gray-800 shadow-lg sm:shadow-2xl rounded-xl sm:rounded-3xl p-4 sm:p-6 md:p-8 border border-gray-200 dark:border-gray-700"
//         >
...
//                 animate={{ x: 0, opacity: 1 }}
//                 transition={{ delay: 0.4 }}
//                 className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white mb-3 sm:mb-4 flex items-center gap-2"
//               >
//                 <Building2 size={isMobile ? 18 : 20} className="text-blue-600 dark:text-blue-400" />
//                 Project Details
//               </motion.h3>
              
//               <div className="grid grid-cols-2 gap-2 sm:gap-4">
//                 <DetailItem label="Company" value={getProjectCompany()} delay={0.5} />
//                 <DetailItem label="Sub Company" value={getProjectSubCompany()} delay={0.55} />
//                 <DetailItem label="Location" value={getProjectLocation()} icon={<MapPin size={12} />} delay={0.6} />
//                 <DetailItem label="Sector" value={getProjectSector()} delay={0.65} />
//                 <DetailItem label="Department" value={getProjectDepartment()} delay={0.7} />
//                 <DetailItem label="Total Length" value={getProjectTotalLength()} icon={<Ruler size={12} />} delay={0.75} />
//                 <DetailItem label="Workorder Cost" value={getProjectCost()} icon={<IndianRupee size={12} />} delay={0.8} />
//                 <DetailItem label="LOA Date" value={getProjectLoaDate()} icon={<Calendar size={12} />} delay={0.85} />
//               </div>
...
//                 <div className="grid grid-cols-2 gap-2 sm:gap-4">
//                   <DetailItem label="Director Proposal" value={getProjectDirectorProposalDate()} delay={0.95} />
//                   <DetailItem label="Project Confirmation" value={getProjectConfirmationDate()} delay={1.0} />
//                 </div>
//               </motion.div>
//             </div>
...
//               className="lg:border-l border-gray-200 dark:border-gray-700 lg:pl-6 sm:pl-0 lg:pl-8"
//             >
...
//                     <span className="text-blue-600 dark:text-blue-400 font-bold">{projectProgress}%</span>
//                   </div>
...
//                       className={`h-full rounded-full ${getProgressColor(projectProgress)}`}
//                     />
//                   </div>
...
//                 {/* Stats */}
//                 <div className="grid grid-cols-2 gap-2 sm:gap-4">
//                   <StatCard label="Total Activities" value={getProjectActivities().length} delay={0.8} />
//                   <StatCard label="Sub Activities" value={totalSubActivities} delay={0.85} />
//                 </div>
...
//                   <motion.span 
//                     whileHover={{ scale: 1.05 }}
//                     className={`px-2 sm:px-3 py-1 rounded-full text-xs font-semibold ${getStatusColor(getProjectStatus())}`}
//                   >
//                     {getProjectStatus()}
//                   </motion.span>
...
//                     <motion.span 
//                       whileHover={{ scale: 1.05 }}
//                       className="px-2 sm:px-3 py-1 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-600 dark:bg-yellow-900/20 dark:text-yellow-400"
//                     >
//                       Extension Requested
//                     </motion.span>
//                   )}
...
//               <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">No activities found</p>
//             )}
//           </motion.div>
...
//                   className="bg-white dark:bg-gray-800 shadow-lg rounded-xl sm:rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-700"
//                 >
//                   {/* Activity Header - Responsive */}
//                   <motion.div 
//                     whileHover={{ backgroundColor: "rgba(59, 130, 246, 0.05)" }}
//                     className="p-3 sm:p-4 md:p-6 bg-gradient-to-r from-gray-50 to-white dark:from-gray-800 dark:to-gray-800"
//                   >
//                     <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
//                       <div className="flex-1 min-w-0">
//                         <div className="flex flex-wrap items-center gap-2 mb-2">
//                           <h4 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white truncate max-w-[200px] sm:max-w-full">
//                             {activity.activity_name || activity.name}
//                           </h4>
//                           <motion.span 
//                             whileHover={{ scale: 1.05 }}
//                             className={`text-xs px-2 py-1 rounded-full whitespace-nowrap ${
//                               isActivityCompleted ? "bg-green-100 text-green-600 dark:bg-green-900/20 dark:text-green-400" : 
//                               daysLeft < 0 ? "bg-red-100 text-red-600 dark:bg-red-900/20 dark:text-red-400" : 
//                               "bg-blue-100 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400"
//                             }`}
//                           >
//                             {isActivityCompleted ? "Completed" : 
...
//                             <motion.span 
//                               whileHover={{ scale: 1.05 }}
//                               className="text-xs px-2 py-1 bg-purple-100 text-purple-600 dark:bg-purple-900/20 dark:text-purple-400 rounded-full"
//                             >
//                               {activity.weightage}%
//                             </motion.span>
//                           )}
...
//                                 }}
//                                 className="p-1 text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors"
...
//                                 whileHover={{ scale: 1.1 }}
//                                 whileTap={{ scale: 0.9 }}
//                                 onClick={() => handleDeleteActivity(activity.id, activity.activity_name || activity.name)}
...
//                           </div>
//                           {/* FIXED: Don't show deadline badge for completed activities */}
//                           {!isActivityCompleted && getDeadlineBadge(activity.end_date || activity.endDate)}
//                         </div>
//                       </div>

//                       <div className="flex items-center justify-between sm:justify-end gap-3 sm:gap-6">
//                         {/* Activity Progress - Hidden on mobile, shown in expanded view */}
//                         {!isMobile && (
...
//                   {/* Expanded Sub Activities - Responsive */}
//                   <AnimatePresence>
//                     {isExpanded && (
//                       <motion.div
...
//                                 }}
//                                 className="bg-blue-600 text-white px-3 py-1.5 rounded-lg hover:bg-blue-700 transition-colors text-xs sm:text-sm flex items-center gap-1"
//                               >
//                                 <Plus size={14} />
//                                 Add Sub-Activity
//                               </motion.button>
...
//                                             value={subStatus}
//                                             onChange={(e) => handleStatusUpdate(activity.id, sub.id, e.target.value)}
//                                             className={`text-xs px-2 py-1 rounded-full border font-semibold ${getStatusColor(subStatus)}`}
//                                           >
...
//                                           <motion.span 
//                                             whileHover={{ scale: 1.05 }}
//                                             className={`text-xs px-2 py-1 rounded-full ${getStatusColor(subStatus)}`}
//                                           >
//                                             {subStatus}
//                                           </motion.span>
//                                         )}
...
//                                                 whileHover={{ scale: 1.1 }}
//                                                 whileTap={{ scale: 0.9 }}
//                                                 onClick={() => handleDeleteSubActivity(activity.id, sub.id, sub.subactivity_name || sub.name)}
//                                                 className="p-1 text-gray-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"
//                                                 title="Delete"
//                                               >
//                                                 <Trash2 size={14} />
...
//                                         {/* FIXED: Don't show deadline badge for completed sub-activities */}
//                                         {!isSubCompleted && subDaysLeft !== null && subDaysLeft > 0 && (
//                                           <motion.span 
//                                             animate={{ scale: [1, 1.05, 1] }}
//                                             transition={{ duration: 1, repeat: Infinity }}
//                                             className={`${
//                                               subDaysLeft <= 2 ? "text-red-600 dark:text-red-400 font-semibold" : 
//                                               subDaysLeft <= 7 ? "text-yellow-600 dark:text-yellow-400" : "text-blue-600 dark:text-blue-400"
//                                             }`}
//                                           >
...
//                                         {/* Progress Bar - FIXED: Now shows correctly */}
//                                         <div className="flex-1">
//                                           <div className="flex justify-between text-xs mb-1">
//                                             <span>Progress</span>
//                                             <span className="font-bold">{subProgress}%</span>
//                                           </div>
...
//                                               transition={{ duration: 0.5 }}
//                                               className={`h-2 rounded-full ${getProgressColor(subProgress)}`}
//                                             />
//                                           </div>
//                                         </div>
...
//                                               onClick={() => {
//                                                 if (subUnit !== "status") {
//                                                   handleProgressUpdate(activity.id, sub.id, subPlannedQty);
//                                                 } else {
...
//                                               setEditValue(subUnit !== "status" ? subCompletedQty : subStatus);
//                                             }}
//                                             className="p-2 text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors self-end sm:self-auto"
...
// const DetailItem = ({ label, value, icon, delay = 0 }) => (
//   <motion.div 
//     initial={{ y: 10, opacity: 0 }}
//     animate={{ y: 0, opacity: 1 }}
//     transition={{ delay }}
...
//     transition={{ delay }}
//     whileHover={{ scale: 1.05 }}
//     className="bg-gray-50 dark:bg-gray-900 p-3 sm:p-4 rounded-xl"
//   >
...
const ProjectDetails = () => {
  ...
  // Calculate project statistics including users
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
      <AnimatePresence>
        {isSubmitting && <LoadingOverlay message={loadingMessage} submessage={loadingSubMessage} />}
      </AnimatePresence>

      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 py-4 sm:py-6 md:py-8 space-y-4 sm:space-y-6 md:space-y-8">
        
        <ActivityExtensionModal isOpen={showActivityExtensionModal} onClose={() => { setShowActivityExtensionModal(false); setSelectedActivityForExtension(null); }} onSubmit={({ newDate, reason }) => handleExtendActivity(selectedActivityForExtension, newDate, reason)} item={getProjectActivities().find(a => a.id === selectedActivityForExtension)} itemType="activity" />
        <ActivityExtensionModal isOpen={showSubActivityExtensionModal} onClose={() => { setShowSubActivityExtensionModal(false); setSelectedSubActivityForExtension(null); }} onSubmit={({ newDate, reason }) => { handleExtendSubActivity(selectedSubActivityForExtension.activityId, selectedSubActivityForExtension.subId, newDate, reason); }} item={{ name: selectedSubActivityForExtension?.subName, endDate: selectedSubActivityForExtension?.endDate, parentActivity: selectedSubActivityForExtension?.activityName }} itemType="subactivity" />

        <AnimatePresence>
          {showAddSubModal && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setShowAddSubModal(false)}>
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} transition={{ type: "spring", damping: 20 }} className="bg-white dark:bg-gray-800 rounded-2xl p-4 sm:p-6 max-w-md w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                <form onSubmit={handleAddSubActivity}>
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-white">Add Sub-Activity</h3>
                  ...
                  <div className="space-y-4">
                    ...
                    {newSubActivity.unit === "Percentage" ? (
                      ...
                      <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.3 }} className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl text-sm text-blue-600 dark:text-blue-400 flex items-center gap-2">
                        <Info size={16} /> Status-based tracking - No quantity needed.
                      </motion.div>
                    )}
                  </div>
                  <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 }} className="flex flex-col sm:flex-row justify-end gap-2 mt-6">
        ...
        <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }} className={`rounded-xl sm:rounded-2xl p-3 sm:p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 ${getProjectStatus() === "DELAYED" ? "bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800" : getProjectStatus() === "COMPLETED" ? "bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800" : "bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800"}`}>
          <div className="flex items-center gap-2 sm:gap-3">
            {getProjectStatus() === "DELAYED" ? <motion.div animate={{ rotate: [0, 10, -10, 0] }} transition={{ duration: 0.5, repeat: Infinity, repeatDelay: 2 }}><AlertCircle className="text-red-600 dark:text-red-400" size={isMobile ? 20 : 24} /></motion.div> : getProjectStatus() === "COMPLETED" ? <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 0.5, repeat: Infinity, repeatDelay: 2 }}><CheckCircle2 className="text-green-600 dark:text-green-400" size={isMobile ? 20 : 24} /></motion.div> : <motion.div animate={{ rotate: 360 }} transition={{ duration: 10, repeat: Infinity, ease: "linear" }}><Clock className="text-blue-600 dark:text-blue-400" size={isMobile ? 20 : 24} /></motion.div>}
            <div>
        ...
        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }} whileHover={{ scale: 1.01 }} className="bg-white dark:bg-gray-800 shadow-lg sm:shadow-2xl rounded-xl sm:rounded-3xl p-4 sm:p-6 md:p-8 border border-gray-200 dark:border-gray-700">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
            <div className="space-y-3 sm:space-y-4">
              <motion.h3 initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.4 }} className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white mb-3 sm:mb-4 flex items-center gap-2">
              ...
              <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.9 }} className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-gray-200 dark:border-gray-700">
                <h4 className="font-semibold text-sm sm:text-base text-gray-800 dark:text-white mb-2 sm:mb-3">Key Milestones</h4>
                <div className="grid grid-cols-2 gap-2 sm:gap-4">
                  <DetailItem label="Director Proposal" value={getProjectDirectorProposalDate()} delay={0.95} />
                  <DetailItem label="Project Confirmation" value={getProjectConfirmationDate()} delay={1.0} />
            ...
            <motion.div initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.5 }} className="lg:border-l border-gray-200 dark:border-gray-700 lg:pl-6 sm:pl-0 lg:pl-8">
              <h4 className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white mb-4 sm:mb-6">Progress Overview</h4>
              <div className="space-y-4 sm:space-y-6">
                ...
                <div className="grid grid-cols-3 gap-2 sm:gap-4">
                  <StatCard label="Total Activities" value={getProjectActivities().length} delay={0.8} />
                  <StatCard label="Total Tasks" value={totalSubActivities} delay={0.85} />
                  <StatCard label="Team Members" value={projectStats.totalUsers} delay={0.9} icon={<Users size={14} />} />
                </div>
                <div className="flex flex-wrap gap-2">
                  <motion.span whileHover={{ scale: 1.05 }} className={`px-2 sm:px-3 py-1 rounded-full text-xs font-semibold ${getStatusColor(getProjectStatus())}`}>{getProjectStatus()}</motion.span>
                  {getProjectExtensionRequested() && <motion.span whileHover={{ scale: 1.05 }} className="px-2 sm:px-3 py-1 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-600 dark:bg-yellow-900/20 dark:text-yellow-400">Extension Requested</motion.span>}
                </div>
              </div>
            </motion.div>
          </div>
              ...
              {projectStats.usersList.map((member, idx) => (
                <motion.div key={member.emp_code} initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 1.1 + idx * 0.05 }} whileHover={{ scale: 1.02 }} className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4 border border-gray-200 dark:border-gray-600">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <button onClick={() => {}} className="mt-2 text-xs text-blue-600 hover:text-blue-700 flex items-center gap-1"><Eye size={12} /> View Tasks</button>
                  </div>
                </motion.div>
              ))}
            </div>
        ...
        {/* Activities Section */}
        <div className="space-y-4 sm:space-y-6">
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 1.0 }} className="flex justify-between items-center">
            <h3 className="text-lg sm:text-xl md:text-2xl font-semibold text-gray-900 dark:text-white flex items-center gap-2">
              <ClipboardList size={isMobile ? 20 : 24} className="text-blue-600 dark:text-blue-400" />
              Activities & Sub-Activities
            </h3>
          ...
          {getProjectActivities().length > 0 ? (
            getProjectActivities().map((activity, index) => {
              ...
              return (
                <motion.div key={activity.id} layout initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 1.1 + index * 0.1 }} className="bg-white dark:bg-gray-800 shadow-lg rounded-xl sm:rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-700">
                  <motion.div whileHover={{ backgroundColor: "rgba(59, 130, 246, 0.05)" }} className="p-3 sm:p-4 md:p-6 bg-gradient-to-r from-gray-50 to-white dark:from-gray-800 dark:to-gray-800">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          ...
                          <motion.span whileHover={{ scale: 1.05 }} className={`text-xs px-2 py-1 rounded-full whitespace-nowrap ${isActivityCompleted ? "bg-green-100 text-green-600 dark:bg-green-900/20 dark:text-green-400" : daysLeft < 0 ? "bg-red-100 text-red-600 dark:bg-red-900/20 dark:text-red-400" : "bg-blue-100 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400"}`}>
                            {isActivityCompleted ? "Completed" : daysLeft < 0 ? "Delayed" : "Ongoing"}
                          </motion.span>
                          {activity.weightage && <motion.span whileHover={{ scale: 1.05 }} className="text-xs px-2 py-1 bg-purple-100 text-purple-600 dark:bg-purple-900/20 dark:text-purple-400 rounded-full">{activity.weightage}%</motion.span>}
                          <div className="flex items-center gap-1 ml-auto sm:ml-0">
                            {(user?.role === "ADMIN" || user?.role === "SUPER_ADMIN") && (
                              <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => { setSelectedActivityForExtension(activity.id); setShowActivityExtensionModal(true); }} className="p-1 text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors" title="Extend deadline"><Calendar size={isMobile ? 14 : 16} /></motion.button>
                            )}
                            {user?.role === "SUPER_ADMIN" && (
                              <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => handleDeleteActivity(activity.id, activity.activity_name || activity.name)} className="p-1 text-gray-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors" title="Delete activity"><Trash2 size={isMobile ? 14 : 16} /></motion.button>
                            )}
                          </div>
                        </div>
                        <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm text-gray-500 dark:text-gray-400">
                      ...
                      <div className="flex items-center justify-between sm:justify-end gap-3 sm:gap-6">
                        {!isMobile && <div className="w-24 sm:w-32"><div className="flex justify-between text-xs mb-1"><span>Progress</span><span className="font-bold">{activityProgress}%</span></div><div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2"><motion.div initial={{ width: 0 }} animate={{ width: `${activityProgress}%` }} transition={{ duration: 0.5 }} className={`h-2 rounded-full ${getProgressColor(activityProgress)}`} /></div></div>}
                        <div className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 whitespace-nowrap">{subActivities.length} sub</div>
                        <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => toggleActivity(activity.id)} className="p-1.5 sm:p-2 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg transition-colors">{isExpanded ? <ChevronUp size={isMobile ? 16 : 20} /> : <ChevronDown size={isMobile ? 16 : 20} />}</motion.button>
                      </div>
                    </div>
                  ...
                  <AnimatePresence>
                    {isExpanded && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
                        <div className="p-3 sm:p-4 md:p-6">
                          {(user?.role === "ADMIN" || user?.role === "SUPER_ADMIN") && (
                            <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }} className="mb-4 flex justify-end">
                              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => { setSelectedActivityForSub(activity.id); setShowAddSubModal(true); }} className="bg-blue-600 text-white px-3 py-1.5 rounded-lg hover:bg-blue-700 transition-colors text-xs sm:text-sm flex items-center gap-1"><Plus size={14} /> Add Sub-Activity</motion.button>
                            </motion.div>
                          )}
                          ...
                          {subActivities.length > 0 ? (
                            <div className="space-y-3 sm:space-y-4">
                              {subActivities.map((sub, subIndex) => {
                                ...
                                return (
                                  <motion.div key={sub.id} initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.1 + subIndex * 0.05 }} className={`bg-white dark:bg-gray-800 p-3 sm:p-4 rounded-xl shadow-sm border ${isSubCompleted ? "border-green-200 dark:border-green-800" : isPicked ? "border-blue-200 dark:border-blue-800" : "border-gray-200 dark:border-gray-700"}`}>
                                    <div className="flex flex-col space-y-3">
                                      <div className="flex flex-wrap items-center gap-2">
                                        ...
                                        {subUnit === "status" ? (
                                          <motion.select whileHover={{ scale: 1.02 }} value={subStatus} onChange={(e) => handleStatusUpdate(activity.id, sub.id, e.target.value)} className={`text-xs px-2 py-1 rounded-full border font-semibold ${getStatusColor(subStatus)}`}>
                                          ...
                                          <motion.span whileHover={{ scale: 1.05 }} className={`text-xs px-2 py-1 rounded-full ${getStatusColor(subStatus)}`}>{subStatus}</motion.span>
                                        )}
                                        
                                        {/* Picked By Badge */}
                                        {isPicked && !isSubCompleted && (
                                          <motion.div whileHover={{ scale: 1.05 }} className={`text-xs px-2 py-1 rounded-full flex items-center gap-1 ${pickedByMe ? "bg-green-100 text-green-600 dark:bg-green-900/20 dark:text-green-400" : "bg-blue-100 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400"}`}>
                                            <UserCheck size={12} />
                                            {pickedByMe ? "Picked by you" : `Picked by ${pickedInfo.emp_name}`}
                                          </motion.div>
                                        )}
                                        ...
                                        {(user?.role === "ADMIN" || user?.role === "SUPER_ADMIN") && (
                                          <div className="flex items-center gap-1 ml-auto">
                                            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => { setSelectedSubActivityForExtension({ activityId: activity.id, activityName: activity.activity_name || activity.name, subId: sub.id, subName: sub.subactivity_name || sub.name, endDate: sub.end_date || sub.endDate }); setShowSubActivityExtensionModal(true); }} className="p-1 text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors" title="Extend deadline"><Calendar size={14} /></motion.button>
                                            {user?.role === "SUPER_ADMIN" && (<motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => handleDeleteSubActivity(activity.id, sub.id, sub.subactivity_name || sub.name)} className="p-1 text-gray-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors" title="Delete"><Trash2 size={14} /></motion.button>)}
                                          </div>
                                        )}
                                      </div>
                                      ...
                                      {/* Progress Bar - FIXED: Now shows correct progress */}
                                      <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
                                        <div className="flex-1">
                                          <div className="flex justify-between text-xs mb-1">
                                            <span>Progress</span>
                                            <span className={`font-bold ${isSubCompleted ? "text-green-600" : "text-blue-600"}`}>{subProgress}%</span>
                                          </div>
                                          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 overflow-hidden">
                                            <motion.div
                                              initial={{ width: 0 }}
                                              animate={{ width: `${subProgress}%` }}
                                              transition={{ duration: 0.5 }}
                                              className={`h-2 rounded-full ${isSubCompleted ? "bg-green-500" : getProgressColor(subProgress)}`}
                                            />
                                          </div>
                                        </div>
                                        ...
                                        {isEditing ? (
                                          <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="flex items-center gap-2">
                                            ...
                                            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => { if (subUnit !== "status") { handleProgressUpdate(activity.id, sub.id, subPlannedQty); } else { handleStatusUpdate(activity.id, sub.id, editValue); } }} className="p-1 bg-green-500 text-white rounded hover:bg-green-600" title="Save"><Save size={14} /></motion.button>
                                            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => setEditingSubActivity(null)} className="p-1 bg-gray-500 text-white rounded hover:bg-gray-600" title="Cancel"><XCircle size={14} /></motion.button>
                                          </motion.div>
                                        ) : (
                                          <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => { setEditingSubActivity(sub.id); setEditValue(subUnit !== "status" ? subCompletedQty : subStatus); }} className="p-2 text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors self-end sm:self-auto" title="Update Progress"><PenLine size={16} /></motion.button>
                                        )}
                                      </div>

                                      {/* Picked By Details */}
                                      {isPicked && (
                                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                                          <div className="flex items-center gap-2 text-xs text-blue-700 dark:text-blue-300">
                                            ...
                                            {pickedInfo.started_at && (
                                              <>
                                                <span className="text-blue-500">•</span>
                                                <span>Started: {new Date(pickedInfo.started_at).toLocaleDateString()}</span>
                                              </>
                                            )}
                                            {pickedInfo.status === 'COMPLETED' && (
                                              <>
                                                <span className="text-blue-500">•</span>
                                          ...
                                          {pickedInfo.completed_quantity > 0 && (
                                            <div className="text-xs text-blue-600 mt-1">
                                              Completed: {pickedInfo.completed_quantity} {subUnit !== "status" ? subUnit : ""}
                                              {pickedInfo.total_time_spent > 0 && ` • Time: ${pickedInfo.total_time_spent.toFixed(1)} hrs`}
                                            </div>
                                          )}
                                        </motion.div>
                                      )}
                            ...
                            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center text-gray-500 dark:text-gray-400 py-4 text-sm">No sub-activities found</motion.p>
                          )}
                        </div>
                      </motion.div>
                    )}
            ...
            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 1.1 }} className="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl p-8 sm:p-12 text-center border border-gray-200 dark:border-gray-700">
              <motion.div animate={{ rotate: [0, 10, -10, 0], scale: [1, 1.1, 1] }} transition={{ duration: 2, repeat: Infinity }}><ClipboardList size={40} className="mx-auto text-gray-300 dark:text-gray-600 mb-4" /></motion.div>
              <h3 className="text-base sm:text-lg font-medium text-gray-700 dark:text-gray-300 mb-2">No Activities Found</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">This project doesn't have any activities yet.</p>
              {(user?.role === "ADMIN" || user?.role === "SUPER_ADMIN") && (
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => navigate(`/projects/${id}/edit`)} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 inline-flex items-center gap-2 text-sm"><Plus size={16} /> Add Activities</motion.button>
              )}
            </motion.div>
          )}
        </div>
...
const DetailItem = ({ label, value, icon, delay = 0 }) => (
  <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay }} className="space-y-0.5 sm:space-y-1">
    <p className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1">{icon && icon}{label}</p>
    <p className="font-medium text-sm sm:text-base text-gray-900 dark:text-white break-words">{value}</p>
  </motion.div>
);

const StatCard = ({ label, value, delay = 0, icon = null }) => (
  <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay }} whileHover={{ scale: 1.05 }} className="bg-gray-50 dark:bg-gray-900 p-3 sm:p-4 rounded-xl">
